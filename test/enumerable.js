...
describe('bloem', function () {
  ...
  describe('#moore', function () {
    it('should return a hoos.', function () {
      expect(bloem.moore(function () { })).to.be.an.instanceof(bloem.Hoos);
    });

    it('should call passed, and update values.', function (done) {
      var
      pomp = bloem.Pomp(),
      status = [['state1'], ['state2'], ['state3']], i = 0,
      data1 = ['data'], j = 1,
      error1 = ['error1'], error2 = ['error2'], error3 = ['error3'],
      moore = bloem.moore(function (state) {
        if (i === status.length) {
          i += 1;
          throw error1;
        } else if (i !== status.length + 1) {
          expect(state).to.equal(status[i - 1]);
        }
        return [data1, function (data) {
          if (i === status.length + 1) throw error2;
          return status[i++];
        }];
      }, status[i++]),
      tuin = bloem.Tuin(function (error, data) {
        ...
        } else if (j === status.length + 2) {
          expect(error).to.equal(error2);
        } else if (j >= status.length + 3) {
          expect(error).to.equal(error3);
          done();
        }
      });
      pomp.connect(moore).connect(tuin);
      pomp.send(1);
      pomp.send(2);
      pomp.send(3);
      pomp.send(4);
  ...
  describe('#mealy', function () {
    it('should return a hoos.', function () {
      expect(bloem.mealy(function () { })).to.be.an.instanceof(bloem.Hoos);
    });
    ...
    it('should call passed, and update values.', function (done) {
      ...
      mealy = bloem.mealy(function (state, data) {
        if (i === status.length) {
          i += 1;
          throw error1;
        } else if (i !== status.length + 1) {
      ...
      pomp.connect(mealy).connect(tuin);
      pomp.send(1);
      pomp.send(2);
      pomp.send(3);
      pomp.raise(error2);
